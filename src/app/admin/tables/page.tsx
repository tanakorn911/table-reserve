...
export default function AdminTablesPage() {
    ...
    const [query, setQuery] = useState({ date: new Date().toISOString().split('T')[0], time: getNextTimeSlot() });
    ...
    const handleDelete = async (id: number) => {
        if (!confirm('ยืนยันการลบโต๊ะนี้? (การลบจะไม่ส่งผลต่อประวัติการจองเก่า แต่จะเลือกไม่ได้ในการจองใหม่)')) return;
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <button
                        onClick={handleCheck}
                        className={`px-4 py-2 rounded-md shadow-sm text-sm font-bold transition-all ${isDirty
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {tables.map((table) => {
                        ...
                        return (
                            <div
                                key={table.id}
                                className={`relative group flex flex-col p-6 rounded-xl border-2 transition-all shadow-sm ${isOccupied
                            ...
                            >
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div className={`p-3 rounded-lg ${isOccupied ? 'bg-red-200 text-red-800' : 'bg-green-200 text-green-800'
                                        }`}>
                                        <span className="text-xl font-bold">{table.id}</span>
                                    </div>
                                    <span className={`px-2 py-1 text-xs font-bold rounded-full border ${isOccupied ? 'bg-red-100 text-red-700 border-red-200' : 'bg-green-100 text-green-700 border-green-200'
                                        }`}>
                                    ...
                                    <div className="mt-auto pt-4 border-t border-red-200">
                                        ...
                                        <div className="bg-white bg-opacity-60 p-3 rounded-md border border-red-100">
                                            ...
                                            <div className="flex items-center text-xs text-gray-600 mt-1">
                                                ...
                                                เริ่ม: {booking.reservation_time.substring(0, 5)} น. ({booking.party_size} ท่าน)
                                            </div>
                                        </div>
                                    </div>
                                )}
