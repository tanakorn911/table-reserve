...
export default function AdminSidebar({ isOpen, onClose }: AdminSidebarProps) {
    ...
    const allMenuItems = [
        ...
        { name: 'รายการจอง', href: '/admin/reservations', icon: CalendarDaysIcon, roles: ['admin', 'staff'] },
        { name: 'เช็คสถานะจอง', href: '/check-status', icon: MagnifyingGlassIcon, roles: ['admin', 'staff'] },
            ...
            `}>
                <div className="flex items-center justify-between h-16 px-6 border-b border-gray-200 bg-[#3b5998]">
                    <span className="text-lg font-bold text-white leading-tight">
                        จองโต๊ะออนไลน์ <span className="text-blue-200 text-xs block opacity-80 uppercase tracking-tighter">({role === 'admin' ? 'Admin' : 'Staff'})</span>
                    </span>
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.name}
                                href={item.href}
                                onClick={() => { if (window.innerWidth < 768) onClose(); }} // Close on click for mobile
                            ...
                            >
                                <item.icon
                                    className={`w-6 h-6 mr-3 transition-colors ${isActive ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-700'
                                        }`}
                                    aria-hidden="true"
                                />
                                {item.name}
