...
export default function DashboardPage() {
    ...
    const StatCard = ({ title, value, icon: Icon, color, subtitle }: any) => (
        <div className="p-6 bg-white rounded-xl shadow-sm border border-gray-100 flex items-start justify-between hover:shadow-md transition-shadow group">
            ...
            <div className={`p-4 rounded-xl ${color} shadow-lg transition-transform group-hover:scale-110`}>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        ข้อมูลการจองสำหรับวันนี้: {new Date().toLocaleDateString('th-TH', {
                            weekday: 'long',
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric'
                ...
                <div className="flex bg-gray-100 p-1 rounded-lg">
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${activeTab === 'today' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                            }`}
                    >
                        วันนี้
                    </button>
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${activeTab === 'week' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                            }`}
                    >
                        สัปดาห์นี้
                    </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 p-6 bg-white rounded-xl shadow-sm border border-gray-200">
                    <h2 className="mb-6 text-lg font-bold text-gray-900 flex items-center">
                        {activeTab === 'today' ? 'ช่วงเวลาที่ลูกค้าแน่น (Peak Hours)' : 'แนวโน้มการจอง (7 วันย้อนหลัง)'}
                    </h2>
                    <div className="h-80">
                        <ResponsiveContainer width="100%" height="100%">
                            {activeTab === 'today' ? (
                                <AreaChart data={hourlyData}>
                                    ...
                                    <Tooltip />
                                    <Area type="monotone" dataKey="pax" stroke="#8884d8" fill="#8884d8" fillOpacity={0.2} name="จำนวนลูกค้า" />
                                </AreaChart>
                            ) : (
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-100">
                        ...
                        <ul className="text-sm text-gray-500 space-y-2 list-disc pl-4">
                            <li>ช่วงเวลา Peak วันนี้คือ <strong>{hourlyData.length > 0 ? hourlyData.reduce((prev, current) => (prev.pax > current.pax) ? prev : current).time : '-'}</strong></li>
