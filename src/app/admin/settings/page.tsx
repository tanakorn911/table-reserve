...
import { ClockIcon, TableCellsIcon, UserGroupIcon, ShieldCheckIcon, CalendarDaysIcon, TrashIcon, PencilIcon } from '@heroicons/react/24/outline';
...
export default function AdminSettingsPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch Holidays
                const holidaysRes = await supabase.from('holidays').select('*').order('holiday_date', { ascending: true });
    ...
    const syncProfiles = async () => {
        if (!confirm('ต้องการดึงรายชื่อพนักงานทั้งหมดจากระบบ Authentication มาลงตารางใหม่หรือไม่?')) return;
    ...
    const toggleRole = async (profileId: string, currentRole: 'admin' | 'staff') => {
        ...
        if (!confirm(`คุณแน่ใจหรือไม่ที่จะเปลี่ยนสิทธิ์เป็น ${newRole === 'admin' ? 'ผู้ดูแลระบบ' : 'พนักงาน'}?`)) {
            return;
        }
        ...
        <div className="space-y-8 max-w-5xl mx-auto pb-12">
                    ...
                    <div className="flex-1 bg-gray-50 rounded-xl border border-gray-100 p-4 mb-6 overflow-y-auto max-h-[400px]">
                        {profilesLoading ? (
                            ...
                            <div className="py-12 text-center">
                                ...
                                <p className="text-xs text-gray-400 mt-2">กรุณารัน SQL เพื่อ Sync ข้อมูล หรือรีเฟรชหน้าจอ</p>
                            </div>
                        ) : (
                            <ul className="space-y-3">
                                {profiles.map(p => (
                                    <li key={p.id} className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm transition-all hover:shadow-md">
                                        <div className="flex justify-between items-start mb-3">
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <span className="font-bold text-gray-900">{p.full_name || 'ไม่ระบุชื่อ'}</span>
                                                    {p.staff_id && <span className="text-[10px] font-mono bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200">{p.staff_id}</span>}
                                                </div>
                                                <div className="text-xs text-gray-500 mt-0.5">{p.email}</div>
                                                <div className="text-xs font-medium text-indigo-600 mt-1">{p.position || 'ยังไม่ระบุตำแหน่ง'}</div>
                                            </div>
                                            <span className={`px-3 py-1 rounded-full text-xs font-bold border ${p.role === 'admin'
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 flex flex-col h-full lg:col-span-2">
                    ...
                    <div className="bg-gray-50 rounded-xl border border-gray-100 p-4 mb-6">
                        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {tables.map(t => (
                                <li key={t.id} className="flex justify-between items-center bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                                    <div className="overflow-hidden">
                                        <span className="font-bold text-gray-900 block truncate">{t.name}</span>
                                        <span className="text-xs text-gray-500 truncate block">{t.description || '-'}</span>
                                    </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 flex flex-col h-full lg:col-span-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 p-6 bg-red-50 rounded-2xl border border-red-100">
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-red-800 uppercase pl-1">เริ่มตั้งแต่วันที่</label>
                            <input
                                type="date"
                                value={holidayDate}
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-red-800 uppercase pl-1">ถึงวันที่ (ไม่บังคับ)</label>
                            <input
                                type="date"
                    ...
                    <div className="bg-gray-50 rounded-xl border border-gray-100 p-4 max-h-[300px] overflow-y-auto">
                        {holidaysLoading ? <p className="text-center py-4">กำลังโหลด...</p> : holidays.length === 0 ? <p className="text-center py-8 text-gray-400 font-medium italic">ยังไม่มีวันหยุดพิเศษที่ตั้งไว้</p> : (
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead>
                                    <tr>
                                        <th className="px-4 py-2 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">วันที่</th>
                                        <th className="px-4 py-2 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">เหตุผล</th>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {holidays.map(h => (
                                        <tr key={h.id}>
                                            <td className="px-4 py-3 text-sm font-bold text-gray-900">{h.holiday_date}</td>
                                            <td className="px-4 py-3 text-sm text-gray-600 font-medium">{h.description || '-'}</td>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 max-w-2xl mx-auto mt-8">
                ...
                <form onSubmit={handleUpdatePassword} className="space-y-6">
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1.5">เปลี่ยนรหัสผ่านผู้ดูแลระบบ</label>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            onChange={e => setStaffFormData((prev: any) => ({ ...prev, full_name: e.target.value }))}
                        />
                    </div>
                    <div>
                        ...
                        <select
                            ...
                            onChange={e => setStaffFormData((prev: any) => ({ ...prev, position: e.target.value }))}
                        >
                    ...
                    <div>
                        ...
                        <div className="flex gap-2">
                            <input
                                ...
                                onChange={e => setStaffFormData((prev: any) => ({ ...prev, staff_id: e.target.value }))}
